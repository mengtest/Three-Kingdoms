...
public class NGUITransformInspector : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		{
			DrawServerOrient((serializedObject.targetObjects[0] as Transform).localEulerAngles.y);
		}
	}

	void DrawServerOrient(float yEular)
	{
		GUILayout.BeginHorizontal();
		{
			GUILayout.Label("Orient");
			EditorGUILayout.SelectableLabel((yEular/180*Mathf.PI).ToString());
		}
	...
	void DrawPosition ()
	{
		GUILayout.BeginHorizontal();
		{
			...
			if (GUILayout.Button("^", GUILayout.Width(12f))) 
			{
				mPos.vector3Value = new Vector3(Mathf.FloorToInt(mPos.vector3Value.x),  Mathf.FloorToInt(mPos.vector3Value.y), 0);
			}
		}
		GUILayout.EndHorizontal();
	}
	...
	void DrawScale (bool isWidget)
	{
		GUILayout.BeginHorizontal();
		{
			...
			Vector3 baseScale = mScale.vector3Value;
			...
			Vector3 scale = mScale.vector3Value;
			
			if (Differs(baseScale.x, scale.x) || Differs(baseScale.y, scale.y) ||
			    Differs(baseScale.z, scale.z))
			{
				// special scale for shuriken
				if (this.target is Transform)
				{
					Vector3  ratio = new Vector3(scale.x / baseScale.x, scale.y / baseScale.y, scale.z / baseScale.z);
					//((Transform)this.target).ScaleParticleSystem(ratio);
				}				
			}

			GUILayout.Button("^", GUILayout.Width(12f));
		}
		GUILayout.EndHorizontal();
	}
